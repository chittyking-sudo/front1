...
export function HomePage({ 
  ...
  return (
    <Layout>
      {/* Hero Section */}
      <div class="bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 text-white">
            ...
            <form action="/explore" method="GET" class="max-w-2xl mx-auto">
              <div class="flex gap-2">
                <input 
                  ...
                  class="flex-1 px-6 py-4 rounded-full text-gray-900 text-lg focus:outline-none focus:ring-4 focus:ring-white/30"
                />
                <button 
                  type="submit" 
                  class="px-8 py-4 bg-white text-indigo-600 rounded-full font-semibold hover:bg-indigo-50 transition"
                >
          ...
          <div class="flex flex-wrap gap-3">
            {conceptTags.map(tag => (
              <a 
                ...
                class="px-6 py-3 rounded-full font-medium transition hover:scale-105 shadow-sm hover:shadow-md"
                style={`background-color: ${tag.color}15; color: ${tag.color}; border: 2px solid ${tag.color}30;`}
              >
                {tag.name}
                <span class="ml-2 text-sm opacity-70">({tag.usage_count})</span>
              </a>
            ))}
          </div>
        </div>
